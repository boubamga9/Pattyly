-- Update check_and_create_trial function to use submitter_payer_id instead of paypal_email
...
CREATE OR REPLACE FUNCTION check_and_create_trial(
  p_profile_id uuid,
  p_email text,
  p_submitter_payer_id text, -- Changed from paypal_email
  p_instagram text,
  p_tiktok text,
...
begin
  ...
  insert into anti_fraud (
    email, submitter_payer_id, instagram, tiktok -- Updated columns
  ) values (
    p_email, p_submitter_payer_id, p_instagram, p_tiktok -- Updated values
  );

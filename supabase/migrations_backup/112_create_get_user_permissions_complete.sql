-- ==============================================
-- Migration 112: Create get_user_permissions_complete RPC
-- ==============================================
-- Cette migration crée un RPC optimisé qui regroupe toutes les permissions
-- en une seule requête, réduisant de 4 requêtes à 1 requête
-- ==============================================

BEGIN;

-- Supprimer l'ancienne fonction si elle existe
DROP FUNCTION IF EXISTS get_user_permissions_complete(UUID, TEXT, TEXT);

-- Créer la fonction optimisée qui regroupe toutes les permissions
...
BEGIN
    ...
    -- 4. Vérifier si l'utilisateur a une méthode de paiement
    SELECT EXISTS(
        SELECT 1 
        FROM payment_links 
        WHERE profile_id = p_profile_id
    ...
    -- 5. Vérifier si l'utilisateur a déjà eu un abonnement
    SELECT EXISTS(
        SELECT 1 
        FROM user_products 
        WHERE profile_id = p_profile_id
    ...
    -- 6. Construire le résultat JSON avec toutes les données du shop
    SELECT json_build_object(
        ...
        'canAddMoreProducts', COALESCE(v_product_count, 0) < CASE 
            WHEN v_plan = 'premium' THEN 999999
            WHEN v_plan = 'exempt' THEN 999999
            ELSE 10
        END,
...
GRANT EXECUTE ON FUNCTION get_user_permissions_complete(UUID, TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_user_permissions_complete(UUID, TEXT, TEXT) TO anon;

-- Add comment
COMMENT ON FUNCTION get_user_permissions_complete(UUID, TEXT, TEXT) IS 
'Get all user permissions in a single optimized query. Returns shop info, plan, product count, and all permission flags. Reduces from 4 queries to 1.';

<script lang="ts">
	...
	async function handleUpdateStripeAccount() {
		...
		try {
			...
			// Format SvelteKit: [{"success":1,"url":2},true,"url"] ou [{"success":1,"error":2},false,"error"]
			if (Array.isArray(actionResult) && actionResult[0] && typeof actionResult[0] === 'object') {
				const refObj = actionResult[0] as { success?: number; url?: number; error?: number };
				
				// Si error est présent, c'est une erreur
				if ('error' in refObj && typeof refObj.error === 'number') {
					const errorMessage = actionResult[refObj.error] as string || 'Erreur lors de la mise à jour du compte Stripe';
					console.error('Error updating Stripe account:', errorMessage);
					alert(errorMessage);
					stripeUpdateLoading = false;
					return;
				}
				
				// Si url est présent, c'est un succès
				if ('url' in refObj && typeof refObj.url === 'number') {
					const url = actionResult[refObj.url] as string;
					if (url && url.startsWith('http')) {
						window.location.href = url;
						return;
					}
			...
			// Fallback: format simple {success: true, url: "..."}
			if (actionResult && typeof actionResult === 'object' && 'url' in actionResult) {
				const url = (actionResult as { url: string }).url;
				if (url && url.startsWith('http')) {
					window.location.href = url;
					return;
				}
			...
			// Si on arrive ici, on n'a pas pu extraire l'URL
			console.error('Error updating Stripe account: URL not found in response', result);
			alert('Erreur: Impossible de récupérer l\'URL de mise à jour. Veuillez réessayer.');
			stripeUpdateLoading = false;
		} catch (err) {

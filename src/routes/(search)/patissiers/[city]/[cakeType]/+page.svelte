<script lang="ts">
	...
	onMount(async () => {
		...
		const localBusinessSchema = {
			...
			url: `https://pattyly.com/patissiers/${cityParam}/${cakeTypeParam}`,
			areaServed: {
				'@type': 'City',
				name: cityName
			}
		...
		const breadcrumbSchema = {
			...
			itemListElement: [
				...
				},
				{
					...
					position: 2,
					name: 'Annuaire',
					item: 'https://pattyly.com/patissiers'
				},
				{
					'@type': 'ListItem',
					position: 3,
					name: cityName,
					item: `https://pattyly.com/patissiers/${cityParam}`
				},
				{
					'@type': 'ListItem',
					position: 4,
					name: cakeTypeName,
					item: `https://pattyly.com/patissiers/${cityParam}/${cakeTypeParam}`
				}
			]
		};
...
<svelte:head>
	...
	<meta property="og:url" content="https://pattyly.com/patissiers/{cityParam}/{cakeTypeParam}" />
	<link rel="canonical" href="https://pattyly.com/patissiers/{cityParam}/{cakeTypeParam}" />
</svelte:head>
	...
	<section class="relative overflow-hidden bg-white pt-24 pb-16 sm:pt-32 sm:pb-24 md:pt-40 md:pb-32">
		...
		<div class="relative mx-auto max-w-6xl px-6 sm:px-8 lg:px-12">
			<div class="mb-12 text-center">
				...
				<div class="mt-6">
					<Button
						href="/patissiers?city={data.city}&type={data.cakeType}"
			...
			<div class="grid gap-6 sm:grid-cols-2">
				...
				<div>
					...
					<div class="space-y-2">
						{#if cakeTypeParam !== 'gateau-anniversaire'}
							<a
								href="/patissiers/{cityParam}/gateau-anniversaire"
						...
						{#if cakeTypeParam !== 'gateau-mariage'}
							<a
								href="/patissiers/{cityParam}/gateau-mariage"
								class="block rounded-lg border border-neutral-200 bg-white p-4 text-sm text-neutral-700 transition-all duration-200 hover:border-[#FF6F61] hover:bg-[#FFE8D6]/20 hover:text-[#FF6F61]"
							>
						...
						{#if cakeTypeParam !== 'cupcakes'}
							<a
								href="/patissiers/{cityParam}/cupcakes"
								class="block rounded-lg border border-neutral-200 bg-white p-4 text-sm text-neutral-700 transition-all duration-200 hover:border-[#FF6F61] hover:bg-[#FFE8D6]/20 hover:text-[#FF6F61]"
							>
								🧁 Cupcakes à {cityName}
							</a>
				...
				<div>
					...
					<div class="space-y-2">
						{#if cityParam !== 'paris'}
							<a
								href="/patissiers/paris/{cakeTypeParam}"
								class="block rounded-lg border border-neutral-200 bg-white p-4 text-sm text-neutral-700 transition-all duration-200 hover:border-[#FF6F61] hover:bg-[#FFE8D6]/20 hover:text-[#FF6F61]"
							>
						...
						{#if cityParam !== 'lyon'}
							<a
								href="/patissiers/lyon/{cakeTypeParam}"
								class="block rounded-lg border border-neutral-200 bg-white p-4 text-sm text-neutral-700 transition-all duration-200 hover:border-[#FF6F61] hover:bg-[#FFE8D6]/20 hover:text-[#FF6F61]"
							>
								📍 {cakeTypeName} à Lyon
							</a>
						...
						{#if cityParam !== 'marseille'}
							<a
								href="/patissiers/marseille/{cakeTypeParam}"
								class="block rounded-lg border border-neutral-200 bg-white p-4 text-sm text-neutral-700 transition-all duration-200 hover:border-[#FF6F61] hover:bg-[#FFE8D6]/20 hover:text-[#FF6F61]"
							>

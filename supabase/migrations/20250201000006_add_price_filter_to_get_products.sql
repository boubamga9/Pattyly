-- Migration pour ajouter le filtrage par prix directement dans la fonction SQL
-- Cela permet d'avoir une pagination correcte et des performances optimales

-- Supprimer toutes les versions existantes de la fonction pour éviter les conflits de surcharge
...
COMMENT ON FUNCTION "public"."get_products_sorted_by_shop_premium"(
    ...
    "p_min_price" numeric,
    "p_max_price" numeric
) IS 'Returns products sorted by shop premium status (premium shop products first) then by hash-based random order (changes daily, based on product_id to mix products across shops) then name. Uses SECURITY DEFINER to read user_products for premium status check, bypassing RLS. Now includes price filtering (p_min_price and p_max_price).';

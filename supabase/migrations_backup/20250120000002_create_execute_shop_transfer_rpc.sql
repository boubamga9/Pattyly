-- Migration pour créer la fonction RPC execute_shop_transfer
-- Cette fonction encapsule toute la logique de transfert de boutique
...
-- Permissions
GRANT EXECUTE ON FUNCTION execute_shop_transfer(UUID, UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION execute_shop_transfer(UUID, UUID, TEXT) TO service_role;

-- Commentaire
COMMENT ON FUNCTION execute_shop_transfer(UUID, UUID, TEXT) IS 'Execute complete shop transfer process - transfers shop ownership and cleans up old user data';
